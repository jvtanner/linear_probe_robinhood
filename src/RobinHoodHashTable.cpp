...
#include <algorithm>
using namespace std;
/* Similar to linear probing, but initiates fairness algorithm
 * involving alotment of distance parameter to ensure fairness
 */
RobinHoodHashTable::RobinHoodHashTable(HashFunction<string> hashFn) {
    hasher = hashFn;
    logicalSize = 0;
    allocatedSize = hashFn.numSlots();
    table = new Slot[allocatedSize];
}
//derstructs table
...
//inserts element if distance grows to greater value than slot's distance
bool RobinHoodHashTable::insert(const string& elem) {
    int bucket = hasher(elem);
    newSlot.value = elem;
    newSlot.isEmpty = false;
    newSlot.distance = 0;
    for (int i = bucket; i < allocatedSize + bucket; i++){
        //mod loops around array
        if (table[i % allocatedSize].isEmpty){
            swap(table[i % allocatedSize], newSlot);
            logicalSize++;
            return true;
        }
        if (table[i % allocatedSize].distance < newSlot.distance){
            swap(table[i % allocatedSize], newSlot);
        }
        else if (table[i % allocatedSize].value == elem){
            return false;
        }
        newSlot.distance++;
    }
...
//checks for both presence of elem and breaks early if distance becomes too great
bool RobinHoodHashTable::contains(const string& elem) const {
    int bucket = hasher(elem);
    int count = 0;
    for (int i = bucket; i < allocatedSize + bucket; i++){
        if (table[i % allocatedSize].isEmpty || table[i % allocatedSize].distance < count){
            return false;
        } else if (table[i % allocatedSize].value == elem){
            return true;
        }
...
//removes elem and backward shifts remaining elems, decreasing distance counts
bool RobinHoodHashTable::remove(const string& elem) {
    int bucket = hasher(elem);
    int count = 0;
    for (int i = bucket; i < allocatedSize + bucket; i++){
        int index = i % allocatedSize;
        if (table[index].isEmpty || table[i % allocatedSize].distance < count){
            return false;
        }
        else if (table[index].value == elem){
...
//helper function to decrease distance counts for remove function
void RobinHoodHashTable::backShift(const int start) {
    for (int i = start; i < allocatedSize + start; i++){
        if (!table[(i+1) % allocatedSize].isEmpty && table[(i+1) % allocatedSize].distance > 0){
            table[(i+1) % allocatedSize].distance--;
            swap(table[i % allocatedSize], table[(i+1) % allocatedSize]);
        }
        else {
            break;
        }
