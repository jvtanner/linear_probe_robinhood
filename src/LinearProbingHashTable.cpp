...
/*Program to organize and store data within a linear probe hash table.
 */

//Constructor: utilizes logical and allocated size
...
bool LinearProbingHashTable::insert(const string& elem) {
    ...
    for (int i = bucket; i < allocatedSize + bucket; i++){
        //marks the first instance of a tombstone encountered
        if (table[i % allocatedSize].isTombstone && tombstoneMark == -1){
            tombstoneMark = i;
        }
        else if (table[i % allocatedSize].isEmpty && tombstoneMark == -1){
            table[i % allocatedSize].value = elem;
            table[i % allocatedSize].isEmpty = false;
            logicalSize++;
            return true;
        }
    ...
    //returns to the first tombstone to insert if empty is found
    if (tombstoneMark >= 0){
        table[tombstoneMark % allocatedSize].value = elem;
        table[tombstoneMark % allocatedSize].isTombstone = false;
        logicalSize++;
        return true;
    }
    return false;
}
//returns true if elem is in table
bool LinearProbingHashTable::contains(const string& elem) const {
    ...
    for (int i = bucket; i < allocatedSize + bucket; i++){
        if (table[i % allocatedSize].isEmpty){
            return false;
        } else if (table[i % allocatedSize].value == elem && !table[i % allocatedSize].isTombstone){
            return true;
        }
    }
    return false;
}
//removes elem if it exists
bool LinearProbingHashTable::remove(const string& elem) {
    ...
    for (int i = bucket; i < allocatedSize + bucket; i++){
        if (!table[i % allocatedSize].isEmpty && table[i % allocatedSize].value == elem && (!table[i % allocatedSize].isTombstone)){
            table[i % allocatedSize].isTombstone = true;
            logicalSize--;
            return true;
        }
    }
